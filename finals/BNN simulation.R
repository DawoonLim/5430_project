...
vec get_bnn_grad_cpp(vec theta, mat X_batch, vec y_batch, double lambda, 
    ...
    // Armadillo mat constructors use column-major order (same as R)
    ...
    // --- 2. Forward Pass ---
    mat Z1 = X_batch * W1;
    // Broadcasting addition for b1 (add b1 to every row)
    Z1.each_row() += b1.t(); 
    mat A1 = tanh(Z1);
    
    mat Z2 = A1 * W2;
    Z2.each_row() += b2.t();
    vec y_pred = Z2; // Output is (Batch, 1)
    
    // --- 3. Backward Pass ---
    vec residual = y_batch - y_pred;
    
    // dL/dW2 = A1.t * (-residual)
    mat grad_W2 = A1.t() * (-residual);
    double grad_b2 = sum(-residual);
    
    // dL/dA1 = (-residual) * W2.t
    mat d_A1 = (-residual) * W2.t();
    
    // dL/dZ1 = dL/dA1 * (1 - A1^2) (Element-wise multiplication)
    ...
    // --- 4. Apply Prior (Regularization) ---
    // Gradient of U = -LogPosterior Gradient
    // grad_U = scale * grad_Likelihood + lambda * theta
    ...
    vec g_b2 = scale * grad_b2 + lambda * b2; // scalar in vector form
...
// (2) Main Algorithm: Basic SGHMC
// [[Rcpp::export]]
mat sghmc_basic_bnn_cpp(mat X, vec y, vec theta_init, int n_iter, 
    ...
    for(int t = 0; t < n_iter; t++) {
        ...
        mat X_batch = X.rows(indices);
        vec y_batch = y.elem(indices);
        
        // Gradient Calculation (C++ Internal)
        vec grad = get_bnn_grad_cpp(theta, X_batch, y_batch, lambda, scale, d_in, d_hidden, d_out);
        
        // Basic Update
        vec noise = randn(d) * noise_std;
        
        // Update r: r = r - eps*grad - eps*C*r + noise
...
# [3] Splitting SGHMC
sghmc_splitting_bnn <- function(X, y, theta_init, n_iter, epsilon, C, lambda, batch_size) {
  d <- length(theta_init); N <- nrow(X)
  samples <- matrix(0, nrow=n_iter, ncol=d)
  theta <- theta_init; r <- rnorm(d, 0, 1)
  
  decay <- exp(-C * epsilon)
  noise_std <- sqrt(1 - exp(-2 * C * epsilon))
  
  idx <- sample(1:N, batch_size)
  grad <- get_bnn_grad(theta, X[idx,], y[idx], lambda, N, batch_size)
  ...
  for(t in 1:n_iter) {
    ...
    noise <- rnorm(d, 0, 1)
    r <- r * decay + noise_std * noise # Friction & Noise part
    theta <- theta + 0.5 * epsilon * r
...
# [4] Adaptive SGHMC (RMSprop Preconditioning)
sghmc_adaptive_bnn <- function(X, y, theta_init, n_iter, epsilon, C, lambda, batch_size, mdecay=0.05) {
  d <- length(theta_init); N <- nrow(X)
  samples <- matrix(0, nrow=n_iter, ncol=d)
  theta <- theta_init; v <- rnorm(d, 0, sqrt(epsilon))
  g_sq <- rep(1e-5, d)
  ...
  for(t in 1:n_iter) {
    ...
    sigma_inv <- 1 / sqrt(g_sq + 1e-10)
    
    noise_std <- sqrt(2 * epsilon * C) * sqrt(sigma_inv)
    noise <- rnorm(d) * noise_std
...
# [5] Cyclical SGHMC
sghmc_cyclical_bnn <- function(X, y, theta_init, n_iter, epsilon_max, cycle_length, batch_size, beta=0.9, lambda=1.0) {
  d <- length(theta_init); N <- nrow(X)
  samples <- matrix(0, nrow=n_iter, ncol=d)
  theta <- theta_init; m <- rep(0, d)
  ...
  for(t in 1:n_iter) {
    ...
    noise_scale <- sqrt(2.0 * epsilon_t * (1-beta))
    m <- beta * m - epsilon_t * grad + rnorm(d) * noise_scale
    theta <- theta + m
    samples[t, ] <- theta
  }
  return(samples)
